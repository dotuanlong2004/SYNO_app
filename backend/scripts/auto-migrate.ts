 ...
 * Automatically creates tables, RLS policies, and triggers
 */
...
BEGIN
    ...
    VALUES (
        ...
        COALESCE(NEW.raw_user_meta_data->>'school_id', '1')
    );
    RETURN NEW;
END;
...
SELECT 'TEST001', 'Nguyễn Văn Test', '10A1', 'LINK123', '1'
...
async function runMigration() {
  console.log('🚀 Starting Supabase Auto-Migration...\n');
  console.log(`📍 URL: ${SUPABASE_URL}`);
  console.log('🔑 Using Service Role Key: present\n');
  ...
  try {
    ...
    if (error) {
      ...
      // Try alternative: run SQL via REST API
    ...
    console.log('   - attendance_spam_logs');
    console.log('   - hardware_scan_debounce');
    ...
    console.log('\n💡 Alternative: Copy this SQL to Supabase SQL Editor:');
    console.log('   https://app.supabase.com/project/_/sql\n');
    
    // Save SQL to file for manual execution
    const sqlFile = path.join(__dirname, '..', 'migrations', 'manual-run.sql');
